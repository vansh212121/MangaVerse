...
import { useUserCollection, useUpdateMangaStatus } from "@/hooks/useUserQueries";
...
const Collections = () => {
  ...
  const { data: userCollection, isLoading, isError, refetch } = useUserCollection();
  ...
  const handleStatusUpdate = (mal_id, newStatus) => {
    updateStatusMutation.mutate({ mal_id, newStatus }, {
      onSuccess: () => toast.success("Status updated successfully!"),
      onError: (error) => toast.error(error.response?.data?.detail || "Failed to update status."),
    });
  };
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-8">
            <div className="text-center py-16">
                ...
                <Button asChild className="bg-gradient-to-r from-primary to-secondary hover:from-primary/80 hover:to-secondary/80">
  ...
  if (isError) {
    return (
        <div className="min-h-screen bg-background">
            ...
            <SakuraPetals />
            <main className="container mx-auto px-4 py-8">
                <EmptyState message="Could not load your collection." onRetry={refetch} />
            </main>
        </div>
    )
  }
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
          <div className="flex flex-wrap gap-2">
            <Badge variant="outline" className="bg-muted/50">Total: {statusCounts.all}</Badge>
            <Badge variant="outline" className="bg-green-500/10 text-green-400 border-green-500/20">Reading: {statusCounts.reading}</Badge>
            <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20">Completed: {statusCounts.completed}</Badge>
            <Badge variant="outline" className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20">Planned: {statusCounts.planned}</Badge>
          </div>
        ...
        {userCollection.length > 0 ? (
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="grid w-full grid-cols-4 lg:w-auto lg:grid-cols-4">
              ...
              <TabsTrigger value="reading">Reading ({statusCounts.reading})</TabsTrigger>
                  ...
                  {getFilteredCollection(status).map((manga, index) => (
                    ...
                    >
                      <div className="relative">
                        <MangaCard manga={{...manga, id: manga.mal_id}} showStatus={true} />
                        <div className="mt-2">
                          <Select
                            value={manga.status}
                            onValueChange={(value) => handleStatusUpdate(manga.mal_id, value)}
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">
              Start building your collection by bookmarking manga from the home page.
            </p>
            <Button asChild className="bg-gradient-to-r from-primary to-secondary hover:from-primary/80 hover:to-secondary/80">
