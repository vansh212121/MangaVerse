...
async def get_user_manga_link(user_id: int, manga_id: int, db: AsyncSession) -> UserMangaLink | None:
    """Check if a specific user has a specific manga in their collection."""
    statement = select(UserMangaLink).where(
        UserMangaLink.user_id == user_id,
        UserMangaLink.manga_id == manga_id
    )
    result = await db.execute(statement)
    return result.scalar_one_or_none()

async def add_manga_to_user_collection(user_id: int, manga_id: int, db: AsyncSession) -> UserMangaLink:
...
async def update_user_manga_status(link: UserMangaLink, status: MangaStatus, db: AsyncSession) -> UserMangaLink:
...
async def get_user_collection_links(user_id: int, db: AsyncSession) -> list[UserMangaLink]:
